...
int Run14AuAuLeptonCombyReco::process_event(PHCompositeNode *TopNode)
{
    ...
    for (int itrk = 0; itrk < event->GetNtrack(); itrk++)
    {
        ...
        if ( conv_reject==1    && hit_assocaition>99  ) conv_reject=2;
        if ( conv_reject==2    && mytrk->GetTOFDPHI()>2.8  ) conv_reject=10;
        if ( conv_reject==10   && mytrk->GetTOFDPHI()>3.2  ) conv_reject=100;
        if ( conv_reject==100  && (mytrk->GetTOFDPHI()>3.8 || mytrk->GetGhost()<5)  ) conv_reject=1000;
        if ( conv_reject==1000 && (mytrk->GetTOFDPHI()>4.2 || mytrk->GetGhost()<4)  ) conv_reject=10000;
        //if ( conv_reject==10   && !(mytrk->GetMinsDphi(0) + mytrk->GetMinsDphi(1)<-2)  ) conv_reject=100;
        //if ( conv_reject==100  && mytrk->GetTOFDPHI()>2.8  ) conv_reject=1000;

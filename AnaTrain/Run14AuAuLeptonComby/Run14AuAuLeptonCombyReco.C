...
int Run14AuAuLeptonCombyReco::process_event(PHCompositeNode *TopNode)
{
    ...
    for (int itrk = 0; itrk < event->GetNtrack(); itrk++)
    {
        ...
        if ( conv_reject==2    && mytrk->GetTOFDPHI()>2.9  ) conv_reject=10;
        if ( conv_reject==10   && mytrk->GetTOFDPHI()>3.0  ) conv_reject=100;
        if ( conv_reject==100  && mytrk->GetTOFDPHI()>3.1  ) conv_reject=1000;
        if ( conv_reject==1000 && (mytrk->GetTOFDPHI()>3.8 || mytrk->GetGhost()<5)  ) conv_reject=10000;

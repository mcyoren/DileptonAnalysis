...
int Run14AuAuLeptonCombyReco::process_event(PHCompositeNode *TopNode)
{
    ...
    for (int itrk = 0; itrk < event->GetNtrack(); itrk++)
    {
        ...
        if ( mytrk->GetMcId()%10 > 5 ) hadron_reject+=10;
        ...
        if (mytrk->GetPtPrime() > pt_trans){    
            ...
            if ( (((TMath::Abs(mytrk->GetMinsDphi(3))<2.5 && TMath::Abs(mytrk->GetMinsDthe(3))<3) ||
                   (TMath::Abs(mytrk->GetMinsDphi(2))<2.5 && TMath::Abs(mytrk->GetMinsDthe(2))<3) ) && 
                   (TMath::Abs(mytrk->GetMinsDphi(1))<2.0 && TMath::Abs(mytrk->GetMinsDthe(1))<3) && 
                   (TMath::Abs(mytrk->GetMinsDphi(0)) < 2 && TMath::Abs(mytrk->GetMinsDthe(0))<3) )) hit_assocaition=10000;
        }else{
            if ( (((TMath::Abs(mytrk->GetMinsDphi(3))<2.5) &&
                   (TMath::Abs(mytrk->GetMinsDphi(2))<2.5) ) && 
                   (TMath::Abs(mytrk->GetMinsDphi(1))<5.0) &&//podgon change to 5
            ...
            if ( (((TMath::Abs(mytrk->GetMinsDphi(3))<2.5 && TMath::Abs(mytrk->GetMinsDthe(3))<3) &&
                   (TMath::Abs(mytrk->GetMinsDphi(2))<2.5 && TMath::Abs(mytrk->GetMinsDthe(2))<3) ) && 
                   (TMath::Abs(mytrk->GetMinsDphi(1))<3.0 && TMath::Abs(mytrk->GetMinsDthe(1))<3) && 
                   (TMath::Abs(mytrk->GetMinsDphi(0))<3.0 && TMath::Abs(mytrk->GetMinsDthe(0))<3) )) hit_assocaition=10000;
        }
        if ( hit_assocaition>99 && ( mytrk->GetPtPrime() > 0.5 || 
             ( mytrk->GetHitCounter(0) > 0 && mytrk->GetHitCounter(1) > 0 && 
               mytrk->GetHitCounter(2) > 0 && mytrk->GetHitCounter(3) > 0 ) ) ) hit_assocaition+=1;
        
        if ( ((int)mytrk->GetEmcdphi_e())%10==0 && !(mytrk->GetMinsDphi(0)<-2 && mytrk->GetMinsDphi(1)>0) && mytrk->GetEmcdz_e()==0 && ((int)mytrk->GetEmcdphi_e())/100<1 ) conv_reject=1;
        if ( conv_reject==1    && hit_assocaition>99  && mytrk->GetTOFDPHI()>2.8 ) conv_reject=10;
        if ( conv_reject==10   && ( mytrk->GetTOFDPHI()>3.2  || (mytrk->GetMinsDphi(0) + mytrk->GetMinsDphi(1)) > -2 ) ) conv_reject=100;//2.4
        //if ( conv_reject==100  && !(mytrk->GetMinsDphi(0) + mytrk->GetMinsDphi(1)<-2)  ) conv_reject=1000;
        if ( conv_reject==100  && ( mytrk->GetTOFDPHI()>3.2  || (mytrk->GetMinsDphi(0) + mytrk->GetMinsDphi(1)) > 0 )   ) conv_reject=1000;
        if ( conv_reject==1000 && ( mytrk->GetTOFDPHI()>3.8  || (mytrk->GetMinsDphi(0) + mytrk->GetMinsDphi(1)) > 0 )   ) conv_reject=10000;
        //if ( conv_reject==1000  && hit_assocaition%10==1  ) conv_reject=10000;
        ...
        //if ( conv_reject==10   && hit_assocaition%10==1  ) conv_reject=100;
        //if ( conv_reject==100  && !(mytrk->GetMinsDphi(0) + mytrk->GetMinsDphi(1)<-2)  ) conv_reject=1000;
        ...
        hit_assocaition = 0;
        if(mytrk->GetGhost()<99) hit_assocaition = 1;
        if(mytrk->GetGhost()<20) hit_assocaition = 100;
        if(mytrk->GetGhost()<10) hit_assocaition = 10000;

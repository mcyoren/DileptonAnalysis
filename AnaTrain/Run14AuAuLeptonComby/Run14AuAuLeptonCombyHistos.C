...
float Run14AuAuLeptonCombyHistos::get_DCA_V0(PHParticle *Type1, const unsigned int i1, PHParticle *Type2, const unsigned int i2)
{
  ...
  const double DCA_pip = DCA_X_pip/fabs(DCA_X_pip) * sqrt(DCA_X_pip*DCA_X_pip + DCA_Y_pip*DCA_Y_pip);
  const double DCA_pim = DCA_X_pim/fabs(DCA_X_pim) * sqrt(DCA_X_pim*DCA_X_pim + DCA_Y_pim*DCA_Y_pim);

  const double DCA = fabs( DCA_pip + DCA_pim);
...
float Run14AuAuLeptonCombyHistos::get_DCA_V5(PHParticle *Type1, const unsigned int i1, PHParticle *Type2, const unsigned int i2)
{
  ...
  const double KEFF_pip = fabs (p1->get_double(Run14AuAuLeptonCombyEnum::KEFF) - 1);
  const double KEFF_pim = fabs (p2->get_double(Run14AuAuLeptonCombyEnum::KEFF) - 1);

  if ( KEFF_pip > 0.2 || KEFF_pim > 0.2 ) return -999;
...
float Run14AuAuLeptonCombyHistos::get_phi_ee(PHParticle *Type1, const unsigned int i1, PHParticle *Type2, const unsigned int i2)
{
  ...
  const double phi_pip = p1->get_double(Run14AuAuLeptonCombyEnum::PHI0);
  const double phi_pim = p2->get_double(Run14AuAuLeptonCombyEnum::PHI0);

  const double dphi = fabs(phi_pip - phi_pim);

  if ( dphi < TMath::Pi() ) return dphi;
  return 2*TMath::Pi() - dphi;
}
